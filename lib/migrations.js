...
import { knex, modules } from './config';
...
export class Migrations {
  ...
  static create(moduleName, migrationName) {
    ...
    return knex.migrate.make(migrationName, Utils.config(moduleName));
  }
  ...
  static list(moduleName) {
    ...
    if(!moduleName) {
      ...
      for(const m in modules) {
        modulesMigrations[m] = fs.readdirSync(modules[m].migrationsDir);
      }
  ...
  static async down(moduleName, migrationName) {
    ...
    let migrationFiles = Migrations.list(moduleName);
    let [migratedFiles] = await knex.migrate.list(Utils.config(moduleName));
  ...
  static _migrateMany({ moduleName, doMigrate = true }) {
    ...
    return doMigrate
      ? knex.migrate.latest(Utils.config(moduleName))
      : knex.migrate.rollback(Utils.config(moduleName), true);
  }
